...
query="http://localhost:3000/createAccount?password=password&login=$name&first_name=Buthor&last_name=Sumiya'"
...
query3="http://localhost:3000/getToken?password=password&login=$name"
result3=$(curl -X PUT $query3)
echo $result3 > tmp
echo $result3
